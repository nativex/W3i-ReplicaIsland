	...
	public void update(
		...
		if (manager != null) {
			...
			if (player != null) {
				...
				final boolean facingPlayer = (Utils.sign(player.getPosition().x - parentObject.getPosition().x) == Utils.sign(parentObject.facingDirection.x));
				final boolean facingDirectionCorrect = (mRequireFacing && facingPlayer) || !mRequireFacing;
				...
				} else if (mDistance.length2() < (mAttackDistance * mAttackDistance) && currentTime > mAttackStartTime + mAttackLength + mAttackDelay && facingDirectionCorrect) {
