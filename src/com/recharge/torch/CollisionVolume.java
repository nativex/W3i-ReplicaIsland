...
/**
 * CollisionVolume describes a volume (rectangle, sphere, etc) used for dynamic collision detection. Volumes can be tested for intersection against other volumes, and can be grown to contain a set of
 * other volumes. The volume itself is stored in object-relative space (in terms of offsets from some origin); when used with game objects the position of the parent object must be passed to a
 * parameter of the intersection test. This means that a single instance of a CollisionVolume and its derivatives is safe to share amongst many game object instances.
 */
