...
public class DrawableBitmap extends DrawableObject {
	...
	/**
	 * Draw the bitmap at a given x,y position, expressed in pixels, with the lower-left-hand-corner of the view being (0,0).
	 * 
	 * @param gl
	 *            A pointer to the OpenGL context
	 * @param x
	 *            The number of pixels to offset this drawable's origin in the x-axis.
	 * @param y
	 *            The number of pixels to offset this drawable's origin in the y-axis
	 * @param scaleX
	 *            The horizontal scale factor between the bitmap resolution and the display resolution.
	 * @param scaleY
	 *            The vertical scale factor between the bitmap resolution and the display resolution.
	 */
	@Override
	public void draw(
			float x,
		...
		if (gl != null && texture != null) {
			...
			if (!cull) {
				...
				((GL11Ext) gl).glDrawTexfOES(snappedX * scaleX, snappedY * scaleY, getPriority(), width * scaleX, height * scaleY);
	...
	/**
	 * Changes the crop parameters of this bitmap. Note that the underlying OpenGL texture's parameters are not changed immediately The crop is updated on the next call to draw(). Note that the image
	 * may be flipped by providing a negative width or height.
	 * 
	 * @param left
	 * @param bottom
	 * @param width
	...
	@Override
	public boolean visibleAtPosition(
			Vector2 position) {
		boolean cull = false;
		if (mViewWidth > 0) {
			if (position.x + mWidth < 0 || position.x > mViewWidth || position.y + mHeight < 0 || position.y > mViewHeight) {
				cull = true;
			}
		}
		return !cull;
	...
	protected final void setFlip(
			boolean horzFlip,
			boolean vertFlip) {
		setCrop(horzFlip ? mWidth : 0, vertFlip ? 0 : mHeight, horzFlip ? -mWidth : mWidth, vertFlip ? -mHeight : mHeight);
	}
}
