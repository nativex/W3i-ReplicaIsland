...
/**
 * A component to include dynamic collision volumes (such as those produced every frame from animating sprites) in the dynamic collision world. Given a set of "attack" volumes and "vulnerability"
 * volumes (organized such that only attack vs vulnerability intersections result in valid "hits"), this component creates a bounding volume that encompasses the set and submits it to the dynamic
 * collision system. Including this component in a game object will allow it to send and receive hits to other game objects.
 */
public class DynamicCollisionComponent extends GameComponent {
	...
	public void update(
		...
		if (collision != null && mBoundingVolume.getRadius() > 0.0f) {
			collision.registerForCollisions((GameObject) parent, mHitReactionComponent, mBoundingVolume, mAttackVolumes, mVulnerabilityVolumes);
		}
	}
