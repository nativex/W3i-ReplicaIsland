...
public class EventReporter implements Runnable {
	...
	public void recordEvent(
		...
		if (REPORT_SERVER != null) {
			try {
				serverAddress = new URL(REPORT_SERVER + "?" + "event=" + event.eventType + "&x=" + event.x + "&y=" + event.y + "&time=" + event.time + "&level=" + URLEncoder.encode(event.level, "UTF-8") + "&version=" + event.version + "&session=" + event.session);
