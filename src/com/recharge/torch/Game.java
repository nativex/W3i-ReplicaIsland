...
import com.recharge.torch.activities.AndouKun;

/**
 * High-level setup object for the AndouKun game engine. This class sets up the core game engine objects and threads. It also passes events to the game thread from the main UI thread.
 */
public class Game extends AllocationGuard {
	...
	/**
	 * Creates core game objects and constructs the game engine object graph. Note that the game does not actually begin running after this function is called (see start() below). Also note that
	 * textures are not loaded from the resource pack by this function, as OpenGl isn't yet available.
	 * 
	 * @param context
	 */
	...
	public void bootstrap(
		...
		if (!mBootstrapComplete) {
			...
			hud.setFuelDrawable(new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_bar), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_bar_bg), 0, 0));
			hud.setLifeDrawable(new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_heart), 0, 0));
			hud.setFadeTexture(longTermTextureLibrary.allocateTexture(R.drawable.black));
			hud.setButtonDrawables(new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_button_fly_disabled), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_button_fly_off), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_button_fly_on), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_button_stomp_off), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_button_stomp_on), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_movement_slider_base), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_movement_slider_button_off), 0, 0), new DrawableBitmap(longTermTextureLibrary.allocateTexture(R.drawable.ui_movement_slider_button_on), 0, 0));
			Texture[] digitTextures = { longTermTextureLibrary.allocateTexture(R.drawable.ui_0), longTermTextureLibrary.allocateTexture(R.drawable.ui_1), longTermTextureLibrary.allocateTexture(R.drawable.ui_2), longTermTextureLibrary.allocateTexture(R.drawable.ui_3), longTermTextureLibrary.allocateTexture(R.drawable.ui_4), longTermTextureLibrary.allocateTexture(R.drawable.ui_5), longTermTextureLibrary.allocateTexture(R.drawable.ui_6), longTermTextureLibrary.allocateTexture(R.drawable.ui_7), longTermTextureLibrary.allocateTexture(R.drawable.ui_8), longTermTextureLibrary.allocateTexture(R.drawable.ui_9) };
			DrawableBitmap[] digits = { new DrawableBitmap(digitTextures[0], 0, 0), new DrawableBitmap(digitTextures[1], 0, 0), new DrawableBitmap(digitTextures[2], 0, 0), new DrawableBitmap(digitTextures[3], 0, 0), new DrawableBitmap(digitTextures[4], 0, 0), new DrawableBitmap(digitTextures[5], 0, 0), new DrawableBitmap(digitTextures[6], 0, 0), new DrawableBitmap(digitTextures[7], 0, 0), new DrawableBitmap(digitTextures[8], 0, 0), new DrawableBitmap(digitTextures[9], 0, 0) };
	...
	protected synchronized void goToLevel(
		...
		BaseObject.sSystemRegistry.levelSystem.loadLevel(level, params.context.getResources().openRawResource(level.resource), mGameRoot);
	...
	public boolean onOrientationEvent(
			float x,
			float y,
			float z) {
		if (mRunning) {
