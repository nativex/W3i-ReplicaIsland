...
/**
 * GameObject defines any object that resides in the game world (character, background, special effect, enemy, etc). It is a collection of GameComponents which implement its behavior; GameObjects
 * themselves have no intrinsic behavior. GameObjects are also "bags of data" that components can use to share state (direct component-to-component communication is discouraged).
 */
public class GameObject extends PhasedObjectManager {
	...
	public final boolean touchingGround() {
		...
		final boolean touching = gameTime > 0.1f && Utils.close(mLastTouchedFloorTime, time.getGameTime(), COLLISION_SURFACE_DECAY_TIME);
		return touching;
	}
	...
	public final boolean touchingCeiling() {
		final TimeSystem time = sSystemRegistry.timeSystem;
		final float gameTime = time.getGameTime();
		final boolean touching = gameTime > 0.1f && Utils.close(mLastTouchedCeilingTime, time.getGameTime(), COLLISION_SURFACE_DECAY_TIME);
		return touching;
	}
	...
	public final boolean touchingLeftWall() {
		final TimeSystem time = sSystemRegistry.timeSystem;
		final float gameTime = time.getGameTime();
		final boolean touching = gameTime > 0.1f && Utils.close(mLastTouchedLeftWallTime, time.getGameTime(), COLLISION_SURFACE_DECAY_TIME);
		return touching;
	}
	...
	public final boolean touchingRightWall() {
		final TimeSystem time = sSystemRegistry.timeSystem;
		final float gameTime = time.getGameTime();
		final boolean touching = gameTime > 0.1f && Utils.close(mLastTouchedRightWallTime, time.getGameTime(), COLLISION_SURFACE_DECAY_TIME);
		return touching;
	}
	...
	public final void setAcceleration(
	...
	public final void setBackgroundCollisionNormal(
			Vector2 normal) {
	...
	public final void setLastTouchedFloorTime(
			float lastTouchedFloorTime) {
	...
	public final void setLastTouchedCeilingTime(
			float lastTouchedCeilingTime) {
	...
	public final void setLastTouchedLeftWallTime(
			float lastTouchedLeftWallTime) {
	...
	public final void setLastTouchedRightWallTime(
			float lastTouchedRightWallTime) {
