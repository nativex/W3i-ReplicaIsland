...
/**
 * A system for calculating collisions between moving game objects. This system accepts collision volumes from game objects each frame and performs a series of tests to see which of them overlap.
 * Collisions are only considered between offending "attack" volumes and receiving "vulnerability" volumes. This implementation works by using a sweep-and-prune algorithm: objects to be considered are
 * sorted in the x axis and then compared in one dimension for overlaps. A bounding volume that encompasses all attack and vulnerability volumes is used for this test, and when an intersection is
 * found the actual offending and receiving volumes are compared. If an intersection is detected both objects receive notification via a HitReactionComponent, if one has been specified.
 */
public class GameObjectCollisionSystem extends BaseObject {
	...
	/**
	 * Adds a game object, and its related volumes, to the dynamic collision world for one frame. Once registered for collisions the object may damage other objects via attack volumes or receive
	 * damage from other volumes via vulnerability volumes.
	 * 
	 * @param object
	 *            The object to consider for collision.
	 * @param reactionComponent
	 *            A HitReactionComponent to notify when an intersection is calculated. If null, the intersection will still occur and no notification will be sent.
	 * @param boundingVolume
	 *            A volume that describes the game object in space. It should encompass all of the attack and vulnerability volumes.
	 * @param attackVolumes
	 *            A list of volumes that can hit other game objects. May be null.
	 * @param vulnerabilityVolumes
	 *            A list of volumes that can receive hits from other game objects. May be null.
	 */
	public void registerForCollisions(
			GameObject object,
		...
		if (record != null && object != null && boundingVolume != null && (attackVolumes != null || vulnerabilityVolumes != null)) {
		...
		for (int x = 0; x < count; x++) {
			...
			for (int y = x + 1; y < count; y++) {
				...
				if (otherPosition.x + other.boundingVolume.getMinXPosition(sOtherFlip) > maxX) {
					...
					final boolean testRequired = (record.attackVolumes != null && other.vulnerabilityVolumes != null) || (record.vulnerabilityVolumes != null && other.attackVolumes != null);
					if (testRequired && record.boundingVolume.intersects(position, sFlip, other.boundingVolume, otherPosition, sOtherFlip)) {
						// These two objects are potentially colliding.
						// Now we must test all attack vs vulnerability boxes.
						final int hit = testAttackAgainstVulnerability(record.attackVolumes, other.vulnerabilityVolumes, position, otherPosition, sFlip, sOtherFlip);
						if (hit != HitType.INVALID) {
							boolean hitAccepted = false;
							if (other.reactionComponent != null) {
								hitAccepted = other.reactionComponent.receivedHit(other.object, record.object, hit);
							}
							if (record.reactionComponent != null) {
								record.reactionComponent.hitVictim(record.object, other.object, hit, hitAccepted);
							}

						}

						final int hit2 = testAttackAgainstVulnerability(other.attackVolumes, record.vulnerabilityVolumes, otherPosition, position, sOtherFlip, sFlip);
						if (hit2 != HitType.INVALID) {
							boolean hitAccepted = false;
							if (record.reactionComponent != null) {
								hitAccepted = record.reactionComponent.receivedHit(record.object, other.object, hit2);
							}
							if (other.reactionComponent != null) {
								other.reactionComponent.hitVictim(other.object, record.object, hit2, hitAccepted);
							}
	...
	/**
	 * Compares the passed list of attack volumes against the passed list of vulnerability volumes and returns a hit type if an intersection is found.
	 * 
	 * @param attackVolumes
	 *            Offensive collision volumes.
	 * @param vulnerabilityVolumes
	 *            Receiving collision volumes.
	 * @param attackPosition
	 *            The world position of the attacking object.
	 * @param vulnerabilityPosition
	 *            The world position of the receiving object.
	 * @return The hit type of the first attacking volume that intersects a vulnerability volume, or HitType.INVALID if no intersections are found.
	 */
				...
				if (hitType != HitType.INVALID) {
					...
					for (int y = 0; y < vulnerabilityCount; y++) {
						...
						if (vulnerableType == HitType.INVALID || vulnerableType == hitType) {
							if (attackVolume.intersects(attackPosition, attackFlip, vulnerabilityVolume, vulnerabilityPosition, vulnerabilityFlip)) {
	...
	private final void drawDebugVolumes(
			CollisionVolumeRecord record) {
		final Vector2 position = record.object.getPosition();
		if (mDrawDebugBoundingVolume) {
			final CollisionVolume boundingVolume = record.boundingVolume;
			sSystemRegistry.debugSystem.drawShape(position.x + boundingVolume.getMinXPosition(sFlip), position.y + boundingVolume.getMinYPosition(sFlip), boundingVolume.getMaxX() - boundingVolume.getMinX(), boundingVolume.getMaxY() - boundingVolume.getMinY(), DebugSystem.SHAPE_CIRCLE, DebugSystem.COLOR_OUTLINE);
		}
		if (mDrawDebugCollisionVolumes) {
			if (record.attackVolumes != null) {
				...
				for (int y = 0; y < attackVolumeCount; y++) {
					...
					sSystemRegistry.debugSystem.drawShape(position.x + volume.getMinXPosition(sFlip), position.y + volume.getMinYPosition(sFlip), volume.getMaxX() - volume.getMinX(), volume.getMaxY() - volume.getMinY(), volume.getClass() == AABoxCollisionVolume.class ? DebugSystem.SHAPE_BOX : DebugSystem.SHAPE_CIRCLE, DebugSystem.COLOR_RED);
				}
			}
			...
			if (record.vulnerabilityVolumes != null) {
				...
				for (int y = 0; y < vulnVolumeCount; y++) {
					...
					sSystemRegistry.debugSystem.drawShape(position.x + volume.getMinXPosition(sFlip), position.y + volume.getMinYPosition(sFlip), volume.getMaxX() - volume.getMinX(), volume.getMaxY() - volume.getMinY(), volume.getClass() == AABoxCollisionVolume.class ? DebugSystem.SHAPE_BOX : DebugSystem.SHAPE_CIRCLE, DebugSystem.COLOR_BLUE);
				}
			}
		}
	}

	public void setDebugPrefs(
			boolean drawBoundingVolumes,
			boolean drawCollisionVolumes) {
	...
	/**
	 * Comparator for game objects that considers the world position of the object's bounding volume and sorts objects from left to right on the x axis.
	 */
	public final static class CollisionVolumeComparator implements Comparator<CollisionVolumeRecord> {
		...
		public int compare(
			...
			if (object1 == null && object2 != null) {
				...
				final float minX1 = object1.object.getPosition().x + object1.boundingVolume.getMinXPosition(sCompareFlip);
				...
				final float minX2 = object2.object.getPosition().x + object2.boundingVolume.getMinXPosition(sCompareFlip);
