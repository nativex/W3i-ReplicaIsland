...
import com.recharge.torch.RenderSystem.RenderElement;

/**
 * GameRenderer the top-level rendering interface for the game engine. It is called by GLSurfaceView and is responsible for submitting commands to OpenGL. GameRenderer receives a queue of renderable
 * objects from the thread and uses that to draw the scene every frame. If no queue is available then no drawing is performed. If the queue is not changed from frame to frame, the same scene will be
 * redrawn every frame. The GameRenderer also invokes texture loads when it is activated.
 */
public class GameRenderer implements GLSurfaceView.Renderer {
	...
	public void onDrawFrame(
		...
		if (mProfileFrameTime > PROFILE_REPORT_DELAY) {
			...
			DebugLog.d("Render Profile", "Average Submit: " + averageSubmitTime + "  Average Draw: " + averageFrameTime + " Objects/Frame: " + averageObjectsPerFrame + " Wait Time: " + averageWaitTime);
	...
	public synchronized void setDrawQueue(
			ObjectManager queue,
			float cameraX,
			float cameraY) {
		mDrawQueue = queue;
	...
	/**
	 * This function blocks while drawFrame() is in progress, and may be used by other threads to determine when drawing is occurring.
	 */
