...
import com.recharge.torch.GameObjectFactory.GameObjectType;
 ...
 * be derived for entirely different responses. This component must exist on an object for that object to respond to dynamic collisions.
 */
public class HitReactionComponent extends GameComponent {
	...
	public void hitVictim(
		...
		if (hitAccepted) {
			...
			if (mDealHitSound != null && (hitType == mDealHitSoundHitType || mDealHitSoundHitType == CollisionParameters.HitType.INVALID)) {
			...
			if (mSpawnOnDealHitObjectType != GameObjectType.INVALID && hitType == mSpawnOnDealHitHitType) {
				final float x = mAlignDealHitObjectToVictimX ? victim.getPosition().x : parent.getPosition().x;
				final float y = mAlignDealHitObjectToVictimY ? victim.getPosition().y : parent.getPosition().y;

				GameObjectFactory factory = sSystemRegistry.gameObjectFactory;
				GameObjectManager manager = sSystemRegistry.gameObjectManager;

				if (factory != null) {
					GameObject object = factory.spawn(mSpawnOnDealHitObjectType, x, y, parent.facingDirection.x < 0.0f);
	...
	public boolean receivedHit(
		...
		if (mGameEventHitType == hitType && mGameEventHitType != CollisionParameters.HitType.INVALID) {
		...
		case CollisionParameters.HitType.HIT:
			...
			if (!mForceInvincibility && !mInvincible && parent.life > 0 && !sameTeam) {
				...
				if (mBounceOnHit && parent.life > 0) {
					...
					newVelocity.set(0.5f * Utils.sign(newVelocity.x), 0.5f * Utils.sign(newVelocity.y));
	...
	public void setPauseOnAttack(
			boolean pause) {
		mPauseOnAttack = pause;
	}

	public void setPauseOnAttackTime(
			float seconds) {
		mPauseOnAttackTime = seconds;
	}

	public void setBounceOnHit(
			boolean bounce) {
		mBounceOnHit = bounce;
	}

	public void setBounceMagnitude(
			float magnitude) {
	...
	public void setDieWhenCollected(
			boolean die) {
		mDieOnCollect = true;
	}

	public void setDieOnAttack(
			boolean die) {
		mDieOnAttack = die;
	}
	...
	public void setPossessionComponent(
			ChangeComponentsComponent component) {
		mPossessionComponent = component;
	}

	public void setInventoryUpdate(
			InventoryComponent.UpdateRecord update) {
		mInventoryUpdate = update;
	}

	public void setLauncherComponent(
	...
	public void setSpawnGameEventOnHit(
			int hitType,
			int gameFlowEventType,
			int indexData) {
	...
	public final void setForceInvincible(
			boolean force) {
		mForceInvincibility = force;
	}

	public final void setTakeHitSound(
			int hitType,
	...
	public final void setDealHitSound(
			int hitType,
			SoundSystem.Sound sound) {
		mDealHitSound = sound;
	...
	public final void setSpawnOnDealHit(
			int hitType,
			GameObjectType objectType,
			boolean alignToVictimX,
