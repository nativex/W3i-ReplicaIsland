...
/**
 * A system for testing positions against "hot spots" embedded in the level tile map data. A level may contain a layer of "hot spots," tiles that provide a hint to the game objects about how to act in
 * that particular area of the game world. Hot spots are commonly used to direct AI characters, or to define areas where special collision rules apply (e.g. regions that cause instant death when
 * entered).
 */
public class HotSpotSystem extends BaseObject {
	...
	public final float getTileCenterWorldPositionX(
			int tileX) {
		float worldX = 0.0f;
		LevelSystem level = sSystemRegistry.levelSystem;
		if (mWorld != null && level != null) {
