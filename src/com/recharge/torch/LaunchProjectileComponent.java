...
/**
 * A component that allows an object to spawn other objects and apply velocity to them at specific intervals. Can be used to launch projectiles, particle effects, or any other type of game object.
 */
public class LaunchProjectileComponent extends GameComponent {
	...
	public void update(
		...
		if (mTrackedProjectileCount < mMaxTrackedProjectiles || !mTrackProjectiles) {
			if (parentObject.getCurrentAction() == mRequiredAction || mRequiredAction == ActionType.INVALID) {
				...
				if (gameTime - mSetStartedTime >= setDelay && (mSetCount < mSetsPerActivation || mSetsPerActivation == -1)) {
	...
	public final void setObjectTypeToSpawn(
			GameObjectFactory.GameObjectType objectTypeToSpawn) {
		mObjectTypeToSpawn = objectTypeToSpawn;
	}

	public final void setOffsetX(
			float offsetX) {
		mOffsetX = offsetX;
	}

	public final void setOffsetY(
			float offsetY) {
		mOffsetY = offsetY;
	}
	...
	public final void setRequiredAction(
			GameObject.ActionType requiredAction) {
		mRequiredAction = requiredAction;
	}

	public final void setDelayBetweenShots(
			float launchDelay) {
		mDelayBetweenShots = launchDelay;
	}

	public final void setDelayBetweenSets(
			float delayBetweenSets) {
		mDelayBetweenSets = delayBetweenSets;
	}

	public final void setDelayBeforeFirstSet(
			float delayBeforeFirstSet) {
		mDelayBeforeFirstSet = delayBeforeFirstSet;
	}

	public final void setShotsPerSet(
			int shotCount) {
		mProjectilesInSet = shotCount;
	}

	public final void setSetsPerActivation(
			int setCount) {
		mSetsPerActivation = setCount;
	}

	public final void enableProjectileTracking(
			int max) {
	...
	public final void setThetaError(
			float error) {
		mThetaError = error;
	}

	public final void setShootSound(
			SoundSystem.Sound shoot) {
		mShootSound = shoot;
	}
