...
public class LauncherComponent extends GameComponent {
	...
	private void fire(
		...
		if (mLaunchEffect != GameObjectFactory.GameObjectType.INVALID) {
			...
			if (factory != null && manager != null) {
				...
				GameObject effect = factory.spawn(mLaunchEffect, position.x + (mLaunchEffectOffsetX * parentObject.facingDirection.x), position.y + (mLaunchEffectOffsetY * parentObject.facingDirection.y), false);
				if (effect != null) {
					manager.add(effect);
				}
			}
	...
	public void setup(
			...
			float magnitude,
			float launchDelay,
			float postLaunchDelay,
			boolean driveActions) {
		mAngle = angle;
	...
	public void setLaunchEffect(
			GameObjectFactory.GameObjectType effectType,
			float offsetX,
			float offsetY) {
