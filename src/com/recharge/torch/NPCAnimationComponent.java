...
import com.recharge.torch.ChannelSystem.Channel;
...
public class NPCAnimationComponent extends GameComponent {
	...
	public void update(
		...
		if (mSprite != null) {
			...
			if (mChannel != null) {
				if (mChannel.value != null && ((ChannelSystem.ChannelBooleanValue) mChannel.value).value) {
	...
	protected boolean shouldMove(
		...
		if (mStopAtWalls) {
			if ((velocity.x < 0.0f && parentObject.touchingLeftWall()) || (velocity.x > 0.0f && parentObject.touchingRightWall())) {
				result = false;
			}
		}
		return result;
	...
	protected boolean shouldTakeHit(
			GameObject parentObject) {
		boolean result = false;
		if (parentObject.getCurrentAction() == ActionType.HIT_REACT && mSprite.findAnimation(TAKE_HIT) != null) {
			result = true;
		}
		return result;
	}
