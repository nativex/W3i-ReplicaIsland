...
public class NPCComponent extends GameComponent {
	...
	public void update(
		...
		if (mReactToHits && mPauseTime <= 0.0f && parentObject.getCurrentAction() == ActionType.HIT_REACT) {
			...
			if (mSpawnGameEventOnDeath && mGameEvent != -1) {
				if (Utils.close(parentObject.getVelocity().x, 0.0f) && parentObject.touchingGround()) {
			...
			return;
		} else if (parentObject.getCurrentAction() == ActionType.INVALID || (!mReactToHits && parentObject.getCurrentAction() == ActionType.HIT_REACT)) {
	...
	private boolean executeCommand(
		...
		case HotSpotSystem.HotSpotType.TALK:
			if (mHitReactComponent != null) {
				if (parentObject.lastReceivedHitType != HitType.COLLECT) {
					mHitReactComponent.setSpawnGameEventOnHit(HitType.COLLECT, mDialogEvent, mDialogIndex);
		...
		case HotSpotSystem.HotSpotType.NPC_GO_TOWARDS_PLAYER:
			...
			if (manager != null) {
				...
				if (player != null) {
					direction = Utils.sign(player.getCenteredPositionX() - parentObject.getCenteredPositionX());
				}
			}
	...
	public void setHitReactionComponent(
