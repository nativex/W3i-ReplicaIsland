...
import com.recharge.torch.HotSpotSystem.HotSpotType;

/**
 * This component implements the "patrolling" behavior for AI characters. Patrolling characters will walk forward on the map until they hit a direction hot spot or a wall, in which case they may
 * change direction. Patrollers can also be configured via this component to attack the player if appropriate conditions are met.
 */
public class PatrolComponent extends GameComponent {
	...
	public void update(
		...
		if (parentObject.getCurrentAction() == ActionType.INVALID || parentObject.getCurrentAction() == ActionType.HIT_REACT) {
		...
		if ((mFlying || parentObject.touchingGround()) && parentObject.life > 0) {
			...
			if (parentObject.getCurrentAction() == GameObject.ActionType.MOVE && mMaxSpeed > 0.0f) {
				...
				if (hotSpotSystem != null) {
					...
					hotSpot = hotSpotSystem.getHotSpot(parentObject.getCenteredPositionX(), parentObject.getPosition().y + 10.0f);
				}
				final float targetVelocityX = parentObject.getTargetVelocity().x;
				final float targetVelocityY = parentObject.getTargetVelocity().y;

				boolean goLeft = (parentObject.touchingRightWall() || hotSpot == HotSpotType.GO_LEFT) && targetVelocityX >= 0.0f;

				boolean goRight = (parentObject.touchingLeftWall() || hotSpot == HotSpotType.GO_RIGHT) && targetVelocityX <= 0.0f;

				boolean pause = (mMaxSpeed == 0.0f) || hotSpot == HotSpotType.GO_DOWN;

				if (mTurnToFacePlayer && player != null && player.life > 0) {
					final float horizontalDelta = player.getCenteredPositionX() - parentObject.getCenteredPositionX();
					...
					if (targetFacingDirection < 0.0f) { // we want to turn to the left
						if (goRight) {
							goRight = false;
							pause = true;
						} else if (targetFacingDirection != Utils.sign(parentObject.facingDirection.x)) {
							goLeft = true;
						}
				...
				if (!mFlying) {
					...
					final boolean goUp = (parentObject.touchingGround() && targetVelocityY < 0.0f) || hotSpot == HotSpotType.GO_UP;

					final boolean goDown = (parentObject.touchingCeiling() && targetVelocityY > 0.0f) || hotSpot == HotSpotType.GO_DOWN;
	...
	private void updateAttack(
		...
		if (visible && parentObject.getCurrentAction() == GameObject.ActionType.MOVE) {
			...
			if (gameTime - mLastAttackTime > mAttackDuration) {
				parentObject.setCurrentAction(GameObject.ActionType.MOVE);
				if (mAttackStopsMovement) {
					parentObject.getTargetVelocity().x = mMaxSpeed * Utils.sign(parentObject.facingDirection.x);
					parentObject.getAcceleration().x = mAcceleration;
				}
			}
		}
	...
	public void setTurnToFacePlayer(
			boolean turn) {
