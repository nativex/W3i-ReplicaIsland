...
public class PlayerComponent extends GameComponent {
	...
	protected void move(
		...
		if (pool != null && input != null) {
			...
			if (dpad.getPressed() || jumpButton.getPressed()) {
				...
				final boolean inTheAir = !mTouchingGround || impulse.y > VERTICAL_IMPULSE_TOLERANCE;
				if (inTheAir) {
				...
				if (parentObject.getVelocity().y + impulse.y > MAX_UPWARD_SPEED && Utils.sign(impulse.y) > 0) {
					impulse.y = (0.0f);
				...
				if (inTheAir) {
					...
					if (Math.abs(currentSpeed) > maxHorizontalSpeed) {
						float postDragSpeed = currentSpeed - (AIR_DRAG_SPEED * timeDelta * Utils.sign(currentSpeed));
	...
	public void update(
		...
		if (mState != State.DEAD && mState != State.WIN) {
			if (parentObject.life <= 0) {
				...
				parentObject.life = 0;
				gotoDead(gameTime);
			} else if (mState != State.HIT_REACT && parentObject.lastReceivedHitType != HitType.INVALID && parentObject.getCurrentAction() == ActionType.HIT_REACT) {
				gotoHitReact(parentObject, gameTime);
			} else {
				HotSpotSystem hotSpot = sSystemRegistry.hotSpotSystem;
				if (hotSpot != null) {
					// TODO: HACK! Unify all this code.
					if (hotSpot.getHotSpot(parentObject.getCenteredPositionX(), parentObject.getPosition().y + 10.0f) == HotSpotSystem.HotSpotType.DIE) {
						parentObject.life = 0;
						gotoDead(gameTime);
					}
				}
	...
	protected void stateMove(
		...
		if (!mGhostActive) {
			...
			} else if (attackButton.getPressed() && mTouchingGround && mGhostDeactivatedTime + GHOST_REACTIVATION_DELAY < time) {
	...
	protected void gotoFrozen(
			GameObject parentObject) {
		mState = State.FROZEN;
		parentObject.setCurrentAction(ActionType.FROZEN);
	}

	protected void stateFrozen(
			float time,
			float timeDelta,
			GameObject parentObject) {
	...
	protected void statePostGhostDelay(
			float time,
			float timeDelta,
			GameObject parentObject) {
	...
	public final void deactivateGhost(
			float delay) {
		mGhostActive = false;
	...
	public final void setInvincibleSwap(
			ChangeComponentsComponent invincibleSwap) {
		mInvincibleSwap = invincibleSwap;
	}

	public final void setHitReactionComponent(
			HitReactionComponent hitReact) {
		mHitReaction = hitReact;
	}

	public final void setInvincibleFader(
			FadeDrawableComponent fader) {
		mInvincibleFader = fader;
	}

	public final void adjustDifficulty(
			GameObject parent,
			int levelAttemps) {
		// Super basic DDA.
