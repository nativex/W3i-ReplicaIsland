...
/**
 * A component that implements the "pop-out" AI behavior. Pop-out characters alternate between hiding and appearing based on their distance from the player. They do not move or normally attack.
 */
			...
			if (player != null) {
				...
				case STATE_VISIBLE:
					...
					if (mDistance.length2() > (mHideDistance * mHideDistance)) {
						mState = STATE_HIDDEN;
					} else if (mDistance.length2() < (mAttackDistance * mAttackDistance) && currentTime > mLastAttackCompletedTime + mAttackDelay) {
