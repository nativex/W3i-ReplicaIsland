...
/**
 * Provides an interface for controlling a sprite with animations. Manages a list of animations and provides a drawable surface with the correct animation frame to a render component each frame. Also
 * manages horizontal and vertical flipping.
 */
public class SpriteComponent extends GameComponent {
			...
			if (currentFrame != null) {
				...
				if (render != null) {
					...
					if (mVisible && currentFrame.texture != null && factory != null) {
						...
						updateFlip(bitmap, parentObject.facingDirection.x < 0.0f, parentObject.facingDirection.y < 0.0f);
				...
				if (mCollisionComponent != null) {
					mCollisionComponent.setCollisionVolumes(currentFrame.attackVolumes, currentFrame.vulnerabilityVolumes);
				}
			}
		}
	...
	public final SpriteAnimation findAnimation(
			int index) {
	...
	public final boolean animationFinished() {
		...
		if (mCurrentAnimation != null && !mCurrentAnimation.getLoop() && mAnimationTime > mCurrentAnimation.getLength()) {
			result = true;
		}
		return result;
	}
	...
	protected final void updateFlip(
	...
	public final void setRenderComponent(
			RenderComponent component) {
		mRenderComponent = component;
	}

	public final void setCollisionComponent(
			DynamicCollisionComponent component) {
	...
	public final void setCurrentAnimationTime(
			float time) {
		mAnimationTime = time;
	}
