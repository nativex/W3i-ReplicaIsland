...
public class CameraSystem extends BaseObject {
	...
	void setTarget(
		...
		if (target != null && mTarget != target) {
			...
			if (mPreInterpolateCameraPosition.length2() < MAX_INTERPOLATE_TO_TARGET_DISTANCE * MAX_INTERPOLATE_TO_TARGET_DISTANCE) {
	...
	public void update(
		...
		if (mTarget != null) {
			...
			if (mTargetChangedTime > 0.0f) {
				...
				mCurrentCameraPosition.x = Lerp.ease(mPreInterpolateCameraPosition.x, targetPosition.x, INTERPOLATE_TO_TARGET_TIME, delta);

				mCurrentCameraPosition.y = Lerp.ease(mPreInterpolateCameraPosition.y, targetPosition.y, INTERPOLATE_TO_TARGET_TIME, delta);
	...
	/**
	 * Snaps a coordinate against the bounds of the world so that it may not pass out of the visible area of the world.
	 * 
	 * @param worldX
	 ...
	 */
	public float snapFocalPointToWorldBoundsY(
			float worldY) {
