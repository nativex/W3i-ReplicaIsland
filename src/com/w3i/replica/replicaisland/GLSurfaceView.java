 ...
 * <p>
 * Typically you use GLSurfaceView by subclassing it and overriding one or more of the View system input event methods. If your application does not need to override event methods then GLSurfaceView
 * can be used as-is. For the most part GLSurfaceView behavior is customized by calling "set" methods rather than by subclassing. For example, unlike a regular View, drawing is delegated to a separate
 * Renderer object which is registered with the GLSurfaceView using the {@link #setRenderer(Renderer)} call.
 * <p>
 * <h3>Initializing GLSurfaceView</h3>
 * All you have to do to initialize a GLSurfaceView is call {@link #setRenderer(Renderer)}. However, if desired, you can modify the default behavior of GLSurfaceView by calling one or more of these
 * methods before calling setRenderer:
 * <ul>
 ...
 * A given Android device may support multiple possible types of drawing surfaces. The available surfaces may differ in how may channels of data are present, as well as how many bits are allocated to
 * each channel. Therefore, the first thing GLSurfaceView has to do when starting to render is choose what type of surface to use.
 * <p>
 * By default GLSurfaceView chooses an available surface that's closest to a 16-bit R5G6B5 surface with a 16-bit depth buffer and no stencil. If you would prefer a different surface (for example, if
 * you do not need a depth buffer) you can override the default behavior by calling one of the setEGLConfigChooser methods.
 * <p>
 * <h4>Debug Behavior</h4>
 * You can optionally modify the behavior of GLSurfaceView by calling one or more of the debugging methods {@link #setDebugFlags(int)}, and {@link #setGLWrapper}. These methods may be called before
 * and/or after setRenderer, but typically they are called before setRenderer so that they take effect immediately.
 * <p>
 ...
 * A GLSurfaceView must be notified when the activity is paused and resumed. GLSurfaceView clients are required to call {@link #onPause()} when the activity pauses and {@link #onResume()} when the
 * activity resumes. These calls allow GLSurfaceView to pause and resume the rendering thread, and also allow GLSurfaceView to release and recreate the OpenGL display.
 * <p>
 * <h3>Handling events</h3>
 * <p>
 * To handle an event you will typically subclass GLSurfaceView and override the appropriate method, just as you would with any other View. However, when handling the event, you may need to
 * communicate with the Renderer object that's running in the rendering thread. You can do this using any standard Java cross-thread communication mechanism. In addition, one relatively easy way to
 * communicate with your renderer is to call {@link #queueEvent(Runnable)}. For example:
 * 
...
public class GLSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
	...
	/**
	 * Set the glWrapper. If the glWrapper is not null, its {@link GLWrapper#wrap(GL)} method is called whenever a surface is created. A GLWrapper can be used to wrap the GL object that's passed to
	 * the renderer. Wrapping a GL object enables examining and modifying the behavior of the GL calls made by the renderer.
	 * <p>
	...
	/**
	 * Set the debug flags to a new value. The value is constructed by OR-together zero or more of the DEBUG_CHECK_* constants. The debug flags take effect whenever a surface is created. The default
	 * value is zero.
	 * 
	 * @param debugFlags
	 *            the new debug flags
	 ...
	 * If this method is called, it must be called before {@link #setRenderer(Renderer)} is called.
	 * <p>
	 * This method only affects the behavior of the default EGLContexFactory and the default EGLConfigChooser. If {@link #setEGLContextFactory(EGLContextFactory)} has been called, then the supplied
	 * EGLContextFactory is responsible for creating an OpenGL ES 2.0-compatible context. If {@link #setEGLConfigChooser(EGLConfigChooser)} has been called, then the supplied EGLConfigChooser is
	 * responsible for choosing an OpenGL ES 2.0-compatible config.
	 * 
	 * @param version
	...
	/**
	 * Set the rendering mode. When renderMode is RENDERMODE_CONTINUOUSLY, the renderer is called repeatedly to re-render the scene. When renderMode is RENDERMODE_WHEN_DIRTY, the renderer only
	 * rendered when the surface is created, or when {@link #requestRender} is called. Defaults to RENDERMODE_CONTINUOUSLY.
	 * <p>
	...
	/**
	 * Request that the renderer render a frame. This method is typically used when the render mode has been set to {@link #RENDERMODE_WHEN_DIRTY}, so that frames are only rendered on demand. May be
	 * called from any thread. Must not be called before a renderer has been set.
	 */
	...
	/**
	 * Inform the view that the activity is paused. The owner of this view must call this method when the activity is paused. Calling this method will pause the rendering thread. Must not be called
	...
	/**
	 * Inform the view that the activity is resumed. The owner of this view must call this method when the activity is resumed. Calling this method will recreate the OpenGL display and resume the
	 ...
	 * <p>
	 * Typically used for implementing debugging and tracing on top of the default GL interface. You would typically use this by creating your own class that implemented all the GL methods by
	 * delegating to another GL instance. Then you could add your own behavior before or after calling the delegate. All the GLWrapper would do was instantiate and return the wrapper GL instance:
	 * 
	 * <pre class="prettyprint">
	 ...
	 * <p>
	 * <h3>Threading</h3>
	 * The renderer will be called on a separate thread, so that rendering performance is decoupled from the UI thread. Clients typically need to communicate with the renderer from the UI thread,
	 * because that's where input events are received. Clients can communicate using any of the standard Java techniques for cross-thread communication, or they can use the
	 * {@link GLSurfaceView#queueEvent(Runnable)} convenience method.
	 * <p>
	 * <h3>EGL Context Lost</h3>
	 * There are situations where the EGL rendering context will be lost. This typically happens when device wakes up after going to sleep. When the EGL context is lost, all OpenGL resources (such as
	 * textures) that are associated with that context will be automatically deleted. In order to keep rendering correctly, a renderer must recreate any lost resources that it still needs. The
	 * {@link #onSurfaceCreated(GL10, EGLConfig)} method is a convenient place to do this.
	 * 
	 * 
	...
	public interface Renderer {
		 ...
		 * <p>
		 * Since this method is called at the beginning of rendering, as well as every time the EGL context is lost, this method is a convenient place to put code to create resources that need to be
		 * created when the rendering starts, and that need to be recreated when the EGL context is lost. Textures are an example of a resource that you might want to create here.
		 * <p>
		 * Note that when the EGL context is lost, all OpenGL resources associated with that context will be automatically deleted. You do not need to call the corresponding "glDelete" methods such as
		 * glDeleteTextures to manually delete these lost resources.
		 * <p>
		 * 
		 * @param gl
		 *            the GL interface. Use <code>instanceof</code> to test if the interface supports GL11 or higher interfaces.
		 * @param config
		...
		/**
		 * Called when the OpenGL context has been lost is about to be recreated. onSurfaceCreated() will be called after onSurfaceLost().
		 * */
		void onSurfaceLost();
	 ...
	 * <p>
	 * This interface must be implemented by clients wishing to call {@link GLSurfaceView#setEGLContextFactory(EGLContextFactory)}
	 */
	 ...
	 * <p>
	 * This interface must be implemented by clients wishing to call {@link GLSurfaceView#setEGLWindowSurfaceFactory(EGLWindowSurfaceFactory)}
	 */
	...
	public interface EGLConfigChooser {
		/**
		 * Choose a configuration from the list. Implementors typically implement this method by calling {@link EGL10#eglChooseConfig} and iterating through the results. Please consult the EGL
	...
	private class EglHelper {
		...
		public boolean swap() {
			...
			/*
			 * Always check for EGL_CONTEXT_LOST, which means the context and all associated data were lost (For instance because the device went to sleep). We need to sleep until we get a new
			 * surface.
			 */
	...
	private static class GLThreadManager {
		...
		/*
		 * Tries once to acquire the right to use an EGL surface. Does not block. Requires that we are already in the sGLThreadManager monitor when this is called.
		 * 
