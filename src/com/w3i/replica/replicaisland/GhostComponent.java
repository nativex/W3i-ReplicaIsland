...
public class GhostComponent extends GameComponent {
		...
		if (parentObject.life > 0) {
			...
			if (input != null) {
				...
				if (mUseOrientationSensor) {
					...
					parentObject.getTargetVelocity().x = tilt.getX() * mMovementSpeed;

					parentObject.getTargetVelocity().y = tilt.getY() * mMovementSpeed;
					...
					parentObject.getTargetVelocity().x = dpad.getX() * mMovementSpeed;
				...
				if (jumpButton.getTriggered(gameTime) && parentObject.touchingGround() && parentObject.getVelocity().y <= 0.0f && !mChangeActionOnButton) {
	...
	public final void releaseControl(
		...
		if (player != null) {
			...
			/*
			 * final InputSystem input = sSystemRegistry.inputSystem; if (input != null) { input.clearClickTriggered(); }
			 */
		}
	...
	public final void setMovementSpeed(
			float movementSpeed) {
		mMovementSpeed = movementSpeed;
	}

	public final void setJumpImpulse(
			float jumpImpulse) {
	...
	public final void setUseOrientationSensor(
			boolean useSensor) {
		mUseOrientationSensor = useSensor;
	}

	public final void setDelayOnRelease(
			float delayOnRelease) {
		mDelayOnRelease = delayOnRelease;
	}

	public final void setKillOnRelease(
			boolean killOnRelease) {
		mKillOnRelease = killOnRelease;
	}

	public final void setTargetAction(
			GameObject.ActionType action) {
		mTargetAction = action;
	}

	public final void setLifeTime(
			float lifeTime) {
		mLifeTime = lifeTime;
	}

	public final void changeActionOnButton(
			GameObject.ActionType pressedAction) {
		mButtonPressedAction = pressedAction;
		mChangeActionOnButton = true;
	}

	public final void setAmbientSound(
			Sound sound) {
		mAmbientSound = sound;
	}
}
