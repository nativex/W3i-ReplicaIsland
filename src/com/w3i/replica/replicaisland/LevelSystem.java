...
public class LevelSystem extends BaseObject {
	...
	public void sendRestartEvent() {
		mGameFlowEvent.post(GameFlowEvent.EVENT_RESTART_LEVEL, 0, sSystemRegistry.contextParameters.context);
	}

	public void sendNextLevelEvent() {
		mGameFlowEvent.post(GameFlowEvent.EVENT_GO_TO_NEXT_LEVEL, 0, sSystemRegistry.contextParameters.context);
	}
	...
	/**
	 * Loads a level from a binary file. The file consists of several layers, including background tile layers and at most one collision layer. Each layer is used to bootstrap related systems and
	 * provide them with layer data.
	 * 
	 * @param stream
	 *            The input stream for the level file resource.
	 * @param tiles
	 *            A tile library to use when constructing tiled background layers.
	 * @param background
					...
					if (type == 0) { // it's a background layer
						...
						if (mWidthInTiles > 0 && mTileWidth > 0) {
							...
							if (mBackgroundObject == null) {
								mBackgroundObject = builder.buildBackground(backgroundIndex, mWidthInTiles * mTileWidth, mHeightInTiles * mTileHeight);
								root.add(mBackgroundObject);
							}

							builder.addTileMapLayer(mBackgroundObject, currentPriority, scrollSpeed, params.gameWidth, params.gameHeight, mTileWidth, mTileHeight, world, tileIndex);
