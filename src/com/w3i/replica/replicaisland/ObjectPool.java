...
/**
 * A general-purpose pool of objects. Objects in the pool are allocated up front and then passed out to requesting objects until the pool is exhausted (at which point an error is thrown). Code that
 * requests objects from the pool should return them to the pool when they are finished. This class is abstract; derivations need to implement the fill() function to fill the pool, and may wish to
 * override release() to clear state on objects as they are returned to the pool.
 */
