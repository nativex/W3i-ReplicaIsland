...
/**
 * Manages a double-buffered queue of renderable objects. The game thread submits drawable objects to the the active render queue while the render thread consumes drawables from the alternate queue.
 * When both threads complete a frame the queues are swapped. Note that this class can manage any number (>=2) of render queues, but increasing the number over two means that the game logic will be
 * running significantly ahead of the rendering thread, which may make the user feel that the controls are "loose."
 */
