...
public class SoundSystem extends BaseObject {
	...
	synchronized public final int play(
			Sound sound,
			boolean loop,
			int priority) {
		int stream = -1;
	...
	private void removeLoopingStream(
			int stream) {
		final int count = mLoopingStreams.length;
		for (int x = 0; x < count; x++) {
	...
	private final int findSound(
			int resource) {
		mSearchDummy.resource = resource;
		return mSounds.find(mSearchDummy, false);
	}

	synchronized public final void setSoundEnabled(
