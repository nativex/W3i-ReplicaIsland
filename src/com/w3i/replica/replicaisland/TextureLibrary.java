 ...
 * and while the texture data itself is streamed in or loaded as necessary.
 */
public class TextureLibrary extends BaseObject {
	...
	/**
	 * Creates a Texture object that is mapped to the passed resource id. If a texture has already been allocated for this id, the previously allocated Texture object is returned.
	 * 
	 * @param resourceID
	 * @return
	 */
	 ...
	 * @return An associated Texture object, or null if there is no associated texture in the library.
	 */
	public Texture getTextureByResource(
			int resourceID) {
	...
	/**
	 * Locates the texture in the hash. This hash uses a simple linear probe chaining mechanism: if the hash slot is occupied by some other entry, the next empty array index is used. This is O(n) for
	 * the worst case (every slot is a cache miss) but the average case is constant time.
	 * 
	 * @param startIndex
	 * @param key
	 * @return
	 */
	private int findFirstKey(
			int startIndex,
			int key) {
		int index = -1;
		for (int x = 0; x < mTextureHash.length; x++) {
