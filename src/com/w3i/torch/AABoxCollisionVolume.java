...
/**
 * An Axis-Aligned rectangular collision volume. This code treats other volumes as if they are also rectangles when calculating intersections. Therefore certain types of intersections, such as sphere
 * vs rectangle, may not be absolutely precise (in the case of a sphere vs a rectangle, for example, a new rectangle that fits the sphere is used to perform the intersection test, so there is some
 * potential for false-positives at the corners). However, for our purposes absolute precision isn't necessary, so this simple implementation is sufficient.
 */
public class AABoxCollisionVolume extends CollisionVolume {
	...
	public AABoxCollisionVolume(float offsetX, float offsetY, float width, float height, int hit) {
		super(hit);
		mBottomLeft = new Vector2(offsetX, offsetY);
		mWidthHeight = new Vector2(width, height);
	}
	...
	/**
	 * Calculates the intersection of this volume and another, and returns true if the volumes intersect. This test treats the other volume as an AABox.
	 * 
	 * @param position
	...
	public boolean intersects(
		...
		final boolean result = boxIntersect(left, right, top, bottom, otherLeft, otherRight, otherTop, otherBottom) || boxIntersect(otherLeft, otherRight, otherTop, otherBottom, left, right, top, bottom);

		return result;
	}

	/** Tests two axis-aligned boxes for overlap. */
	private boolean boxIntersect(
			float left1,
			float right1,
			float top1,
			float bottom1,
