...
public class BackgroundCollisionComponent extends GameComponent {
	...
	/**
	 * Sets up the collision bounding box. This box may be a different size than the bounds of the sprite that this object controls.
	 * 
	 * @param width
	 ...
	 * @param horzOffset
	 *            The offset of the collision box from the object's origin in the x axis.
	 * @param vertOffset
	 *            The offset of the collision box from the object's origin in the y axis.
	 */
	...
	/**
	 * This function is the meat of the collision response logic. Our collision detection and response must be capable of dealing with arbitrary surfaces and must be frame rate independent (we must
	 * sweep the space in-between frames to find collisions reliably). The following algorithm is used to keep the collision box out of the collision world. 1. Cast a ray from the center point of the
	 * box at its position last frame to the edge of the box at its current position. If the ray intersects anything, snap the box back to the point of intersection. 2. Perform Step 1 twice: once
	 * looking for surfaces opposing horizontal movement and again for surfaces opposing vertical movement. These two ray tests approximate the movement of the box between the previous frame and this
	 * one. 3. Since most collisions are collisions with the ground, more precision is required for vertical intersections. Perform another ray test, this time from the top of the box's position
	 * (after snapping in Step 2) to the bottom. Snap out of any vertical surfaces that the ray encounters. This will ensure consistent snapping behavior on incline surfaces. 4. Add the normals of the
	 * surfaces that were hit up and normalize the result to produce a direction describing the average slope of the surfaces that the box is resting on. Physics will use this value as a normal to
	 * resolve collisions with the background.
	 */
	@Override
	public void update(
			float timeDelta,
		...
		if (mPreviousPosition.length2() != 0) {
			...
			if (collision != null) {
				...
				if (Math.abs(mDelta.x) > Math.abs(mDelta.y)) {
					horizontalHit = sweepHorizontal(mPreviousCenter, mCurrentPosition, mDelta, left, right, centerOffsetY, mHorizontalHitPoint, mHorizontalHitNormal, parentObject);
					verticalHit = sweepVertical(mPreviousCenter, mCurrentPosition, mDelta, bottom, top, centerOffsetX, mVerticalHitPoint, mVerticalHitNormal, parentObject);
				...
				if (level != null) {
					...
					/*
					 * if (mCurrentPosition.y + bottom < 0.0f) { mCurrentPosition.y = (-bottom + 1); verticalHit = true; mVerticalHitNormal.y = (mVerticalHitNormal.y + 1.0f);
					 * mVerticalHitNormal.normalize(); } else
					 */if (mCurrentPosition.y + top > level.getLevelHeight()) {
				...
				if (mDelta.x != 0.0f && mDelta.y != 0.0f) {
					...
					if (collision.castRay(mRayStart, mRayEnd, mFilterDirection, mVerticalHitPoint, mVerticalHitNormal, parentObject)) {
					...
					if (collision.castRay(mRayStart, mRayEnd, mFilterDirection, mHorizontalHitPoint, mHorizontalHitNormal, parentObject)) {
	...
	protected boolean sweepHorizontal(
		...
		if (!Utils.close(delta.x, 0.0f)) {
			...
			if (collision.castRay(previousPosition, mTestPointEnd, mFilterDirection, hitPoint, hitNormal, parentObject)) {
				// snap
	...
	protected boolean sweepVertical(
			Vector2 previousPosition,
			Vector2 currentPosition,
			Vector2 delta,
			int bottom,
