 ...
 * 
 * This class also provides a system for runtime-generated collision segments. These temporary segments are cleared each frame, and consequently must be constantly re-submitted if they are intended to
 * persist. Temporary segments are useful for dynamic solid objects, such as moving platforms.
 * 
 * CollisionSystem.TileVisitor is an interface for traversing individual collision tiles. Ray casts can be used to run user code over the collision world by passing different TileVisitor
 * implementations to executeRay. Provided is TileTestVisitor, a visitor that compares the segments of each tile visited with the ray and searches for points of intersection.
 * 
 */
public class CollisionSystem extends BaseObject {
	...
	/**
	 * Casts a ray into the collision world. The ray is bound by the start and end points supplied. The first intersecting segment that is found is returned; in the case where more than one segment is
	 * found, the segment closest to the start point is returned.
	 * 
	 * @param startPoint
	 *            The starting point for the ray in world units.
	 * @param endPoint
	 *            The end point for the ray in world units.
	 * @param movementDirection
	 *            If set, only segments with normals that oppose this direction will be counted as valid intersections. If null, all intersecting segments will be considered valid.
	 * @param hitPoint
	 *            The point of intersection between a ray and a surface, if one is found.
	 * @param hitNormal
	 *            The normal of the intersecting surface if an intersection is found.
	 * @param excludeObject
	 *            If set, dynamic surfaces from this object will be ignored.
	...
	public boolean castRay(
		...
		if (mCollisionTiles != null && executeRay(startPoint, endPoint, hitPoint, hitNormal, mTileSegmentTester) != -1) {
			hit = true;
		}
		...
		if (mTemporarySegments.getCount() > 0) {
			...
			if (testSegmentAgainstList(mTemporarySegments, startPoint, endPoint, tempHitPoint, tempHitNormal, movementDirection, excludeObject)) {
				if (hit) {
	...
	public boolean testBox(
			...
			float bottom,
			Vector2 movementDirection,
			FixedSizeArray<HitPoint> hitPoints,
			GameObject excludeObject,
			boolean testDynamicSurfacesOnly) {
		...
		if (!testDynamicSurfacesOnly) {
			...
			for (int y = startTileY; y != endTileY + yIncrement; y += yIncrement) {
				for (int x = startTileX; x != endTileX + xIncrement; x += xIncrement) {
					...
					if (tileIndex >= 0 && tileIndex < mCollisionTiles.length && mCollisionTiles[tileIndex] != null) {
						...
						boolean hit = testBoxAgainstList(mCollisionTiles[tileIndex].segments, tileSpaceLeft, tileSpaceRight, tileSpaceTop, tileSpaceBottom, movementDirection, excludeObject, worldTileOffset, hitPoints);
		...
		// temporary segments
		boolean tempHit = testBoxAgainstList(mTemporarySegments, left, right, top, bottom, movementDirection, excludeObject, Vector2.ZERO, hitPoints);
	...
	public void addTemporarySurface(
			Vector2 startPoint,
			Vector2 endPoint,
			Vector2 normal,
	...
	/**
	 * Shoots a ray through the collision world. This function is similar to executeRay() below, except that it is optimized for straight lines (either completely horizontal or completely vertical).
	 * 
	 * @param startPoint
	 *            The starting point for the ray, in world space.
	 * @param endPoint
	 *            The ending point for the ray in world space.
	 * @param hitPoint
	 *            Set to the intersection coordinates if an intersection is found.
	 * @param hitNormal
	 *            Set to the normal of the intersecting surface if an intersection is found.
	 * @param visitor
	 *            Class defining what work to perform at each tile step.
	 * @return The index of the tile that intersected the ray, or -1 if no intersection was found.
	 */
	protected int executeStraigtRay(
		...
		for (int x = 0; x < distance; x++) {
			...
			if (tileIndex >= 0 && tileIndex < mCollisionTiles.length && mCollisionTiles[tileIndex] != null) {
				if (visitor.visit(mCollisionTiles[tileIndex], startPoint, endPoint, hitPoint, hitNormal, currentX, currentY)) {
					hitTile = tileIndex;
					break;
				}
			}
	...
	/**
	 * Shoots a ray through the collision world. Since the collision world is a 2D array of tiles, this algorithm traces a line in tile space and tests against each non-empty tile it visits. The
	 * Bresenham line algorithm is used for the actual traversal, but the action taken at each tile is defined by the visitor class passed to this function.
	 * 
	 * @param startPoint
	 *            The starting point for the ray, in world space.
	 * @param endPoint
	...
	protected int executeRay(
		...
		if (deltaX == 0 || deltaY == 0) {
			hitTile = executeStraigtRay(startPoint, endPoint, startTileX, startTileY, endTileX, endTileY, deltaX, deltaY, hitPoint, hitNormal, visitor);
		} else {
	...
	protected final int worldToTileColumn(
			final float x,
			final int width) {
		return Utils.clamp((int) Math.floor(x / mTileWidth), 0, width - 1);
	}

	protected final int worldToTileRow(
			float y,
			final int height) {
	...
	/*
	 * Given a list of segments and a ray, this function performs an intersection search and returns the closest intersecting segment, if any exists.
	 */
	protected static boolean testSegmentAgainstList(
			FixedSizeArray<LineSegment> segments,
			Vector2 startPoint,
			Vector2 endPoint,
			Vector2 hitPoint,
			Vector2 hitNormal,
		...
		for (int x = 0; x < count; x++) {
			...
			// surfaces.
			final float dot = movementDirection.length2() > 0.0f ? movementDirection.dot(segment.mNormal) : -1.0f;

			if (dot < 0.0f && (excludeObject == null || segment.owner != excludeObject) && segment.calculateIntersection(startPoint, endPoint, hitPoint)) {
	...
	protected static boolean testBoxAgainstList(
			FixedSizeArray<LineSegment> segments,
			float left,
			float right,
			float top,
		...
		for (int x = 0; x < count && hitCount < maxSegments; x++) {
			...
			if (dot < 0.0f && (excludeObject == null || segment.owner != excludeObject) && segment.calculateIntersectionBox(left, right, top, bottom, tempHitPoint)) {
	...
	/*
	 * Loads line segments from a binary file and builds the tiled collision database accordingly.
	 */
	public boolean loadCollisionTiles(
			InputStream stream) {
	...
	/**
	 * An interface for visiting tiles during a ray cast. Implementations of TileVisitor can be passed to executeRay(); the visit() function will be invoked for each tile touched by the ray until the
	 * traversal is completed or visit() returns false.
	 */
	...
	/**
	 * TileTestVisitor tests the ray against a list of segments assigned to each tile. If any segment in any tile of the ray's path is found to be intersecting with the ray, traversal stops and
	 * intersection information is recorded.
	 */
	protected class TileTestVisitor extends TileVisitor {
		...
		/**
		 * Sets the visitor up for a ray test. Initializes the size of the tiles and the direction of movement by which intersections should be filtered.
		 */
		public void setup(
				Vector2 movementDirection,
				int tileWidth,
		...
		/**
		 * Converts the ray into tile space and then compares it to the segments stored in the current tile.
		 */
		@Override
		public boolean visit(
			...
			boolean foundHit = testSegmentAgainstList(tile.segments, mTileSpaceStart, mTileSpaceEnd, hitPoint, hitNormal, mDelta, null);
	...
	/**
	 * A class describing a single surface in the collision world. Surfaces are stored as a line segment and a normal. The normal must be normalized (its length must be 1.0) and should describe the
	 * direction that the segment "pushes against" in a collision.
	 */
	protected class LineSegment extends AllocationGuard {
		...
		/**
		 * Checks to see if these lines intersect by projecting one onto the other and then assuring that the collision point is within the range of each segment.
		 */
		...
		public boolean calculateIntersectionBox(
				float left,
				float right,
				float top,
				float bottom,
