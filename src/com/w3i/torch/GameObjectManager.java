 ...
 * that the position of the camera can be used to determine which objects should receive processing time and which should be ignored. Objects that do not move should have an activation radius that
 * defines a sphere similar to the size of the screen; they only need processing when they are visible. Objects that move around will probably need larger regions so that they can leave the visible
 * area of the game world and not be immediately deactivated.
 */
	...
	public void update(
		...
		if (count > 0) {
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				if (distance2 < (gameObject.activationRadius * gameObject.activationRadius) || gameObject.activationRadius == -1) {
	...
	private final static class HorizontalPositionComparator implements Comparator<BaseObject> {
		...
		public int compare(
			...
			if (object1 == null && object2 != null) {
				...
				float delta = ((GameObject) object1).getPosition().x - ((GameObject) object2).getPosition().x;
				if (delta < 0) {
					result = -1;
				} else if (delta > 0) {
					result = 1;
