...
/**
 * This component allows objects to die and be deleted when their life is reduced to zero or they meet other configurable criteria.
 */
public class LifetimeComponent extends GameComponent {
	...
	public void setDieWhenInvisible(
			boolean die) {
		mDieWhenInvisible = die;
	}

	public void setTimeUntilDeath(
			float time) {
		mTimeUntilDeath = time;
	}

	public void setObjectToSpawnOnDeath(
			GameObjectFactory.GameObjectType type) {
		mSpawnOnDeathType = type;
	}

	public void setIncrementEventCounter(
			int event) {
	...
	public void update(
		...
		if (mDieWhenInvisible) {
			...
			final float dx = Math.abs(parentObject.getPosition().x - camera.getFocusPositionX());
			final float dy = Math.abs(parentObject.getPosition().y - camera.getFocusPositionY());
	...
	private void die(
		...
		if (mSpawnOnDeathType != GameObjectFactory.GameObjectType.INVALID) {
			GameObject object = factory.spawn(mSpawnOnDeathType, parentObject.getPosition().x, parentObject.getPosition().y, parentObject.facingDirection.x < 0.0f);
	...
	public final void setTrackingSpawner(
			LaunchProjectileComponent spawner) {
		mTrackingSpawner = spawner;
	}

	public final void setReleaseGhostOnDeath(
			boolean release) {
		mReleaseGhostOnDeath = release;
	}

	public final void setVulnerableToDeathTiles(
			boolean vulnerable) {
		mVulnerableToDeathTiles = vulnerable;
	}

	public final void setDieOnHitBackground(
			boolean die) {
