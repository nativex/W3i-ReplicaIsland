...
/**
 * ObjectManagers are "group nodes" in the game graph. They contain child objects, and updating an object manager invokes update on its children. ObjectManagers themselves are derived from BaseObject,
 * so they may be strung together into a hierarchy of objects. ObjectManager may be specialized to implement special types of traversals (e.g. PhasedObjectManager sorts its children).
 */
public class ObjectManager extends BaseObject {
	...
	/**
	 * Finds a child object by its type. Note that this may invoke the class loader and therefore may be slow.
	 * 
	 * @param classObject
	 *            The class type to search for (e.g. BaseObject.class).
	 * @return
	 */
	public <T> T findByClass(
			Class<T> classObject) {
		T object = null;
		final int count = mObjects.getCount();
		for (int i = 0; i < count; i++) {
