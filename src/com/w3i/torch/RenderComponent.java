...
/**
 * Implements rendering of a drawable object for a game object. If a drawable is set on this component it will be passed to the renderer and drawn on the screen every frame. Drawable objects may be
 * set to be "camera-relative" (meaning their screen position is relative to the location of the camera focus in the scene) or not (meaning their screen position is relative to the origin at the
...
public class RenderComponent extends GameComponent {
	...
	public void setCameraRelative(
			boolean relative) {
		mCameraRelative = relative;
	}

	public void setDrawOffset(
			float x,
			float y) {
		mDrawOffset.set(x, y);
	}
